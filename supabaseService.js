...
export async function fetchRows(table, filters = {}, order = { column: 'id', ascending: true }) {
...
export async function fetchRowsByBookingId(table, bookingId, order = { column: 'id', ascending: true }) {
    return fetchRows(table, { booking_id: bookingId }, order);
}
...
function calculateArrivalDay(arrivalDate, qurbaniDate) {
...
export async function createAnimal(formData) {
    const payload = {
        ...
        arrival_day: calculateArrivalDay(formData.arrivalDate, formData.qurbaniDate),
        purchase_price: roundToTwo(formData.purchasePrice),
        weight: formData.weight ?? null,
        location: formData.location ?? null,
        health_notes: formData.healthNotes ?? null
    };
    const rows = await insertRow('animals', payload);
    return rows?.[0] ?? null;
}
...
export async function createFeedBatch(formData) {
    const payload = {
        booking_id: formData.bookingId,
        day: formData.day,
...
export async function createAnimalTransportAssignment(animalId, truckId) {
    const payload = {
        animal_id: animalId,
        truck_id: truckId
    };
...
export async function fetchAnimals(bookingId) {
    return fetchRowsByBookingId('animals', bookingId, { column: 'id', ascending: true });
}

export async function fetchTrucks(bookingId) {
    const { data, error } = await supabase.from('trucks').select('*').eq('booking_id', bookingId).order('id', { ascending: true });
    if (error) throw error;
    return data || [];
}

export async function fetchExpenses(bookingId) {
    const { data, error } = await supabase.from('expenses').select('*').eq('booking_id', bookingId).order('id', { ascending: true });
    if (error) throw error;
    return data || [];
}

export async function fetchFeedBatches(bookingId) {
    const { data, error } = await supabase.from('feed_batches').select('*').eq('booking_id', bookingId).order('day', { ascending: true });
    if (error) throw error;
    return data || [];
}

export async function fetchAssignments(bookingId) {
    const trucks = await fetchTrucks(bookingId);
    if (!trucks.length) return [];
    const truckIds = trucks.map((truck) => truck.id);
    const { data, error } = await supabase.from('animal_transport_assignments').select('*').in('truck_id', truckIds);
    if (error) throw error;
    return data || [];
}

function buildAnimalGroups(animals) {
    const groups = {};
...
function buildFeedPeriods(feedBatches) {
    ...
    return sorted
        .map((batch, index) => {
            ...
            return { startDay, endDay, totalCost: Number(batch.total_cost), duration };
        })
...
export function calculateFeedDistribution(animals, feedBatches) {
    if (!feedBatches.length) {
        return { results: [], grandTotal: 0, averagePerAnimalCost: 0, totalFeedCost: 0 };
    }
    ...
    periods.forEach((period) => {
        const periodInfo = groups.map((group) => {
            ...
            return { group, daysPresent, animalDays: group.animalsCount * daysPresent };
        });
        const totalPeriodAnimalDays = periodInfo.reduce((sum, item) => sum + item.animalDays, 0);
        if (!totalPeriodAnimalDays) return;
        const periodRate = period.totalCost / totalPeriodAnimalDays;
        periodInfo.forEach((item) => {
            if (!item.animalDays) return;
            const result = groupResults.find((r) => r.arrivalDay === item.group.arrivalDay);
            if (!result) return;
...
function findGroupForAnimal(animal, groups) {
    return groups.find((group) => group.arrivalDay === animal.arrival_day) || null;
}

function calculateTotalAnimalDays(groups) {
    return groups.reduce((sum, group) => sum + group.animalsCount * group.days, 0);
}

function calculateFixedCostShare(expenses, totalAnimals) {
    if (!totalAnimals) return 0;
...
function calculateRentCostForAnimal(animal, groups, expenses) {
    ...
    if (!totalAnimalDays) return 0;
    return roundToTwo((rentTotal * group.days) / totalAnimalDays);
}

function calculateMedicineCostForAnimal(animalId, expenses) {
    return roundToTwo(
        expenses
...
function calculateExtraCostForAnimal(animal, animals, groups, expenses) {
    const group = findGroupForAnimal(animal, groups);
    if (!group) return 0;
    const totalAnimalDays = calculateTotalAnimalDays(groups);
    if (!totalAnimalDays) return 0;
    ...
    expenses
        ...
        .forEach((expense) => {
            if (expense.distribution_type === 'equal') {
                ...
                const customIds = Array.isArray(expense.custom_animal_ids)
...
export function calculateTransportCost(animalId, trucks, assignments) {
    ...
    return assignedAnimals > 0 ? roundToTwo(Number(truck.total_cost) / assignedAnimals) : 0;
}

export async function fetchCostBreakdown(bookingId) {
